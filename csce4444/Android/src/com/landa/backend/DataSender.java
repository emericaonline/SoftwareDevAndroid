		...
		try {
			...
			while (count >= 0) {
				...
				nameValuePairs.add(new BasicNameValuePair("value",
						"TableId, Title, Price, Modifiables"));
				nameValuePairs.add(new BasicNameValuePair("data", "'"
						+ AsyncHelper.getTable() + "'," + "'"
						+ CustomerOrderFragment.order.get(count).getName()
						+ "','"
						+ CustomerOrderFragment.order.get(count).getPrice()
						+ "','"
						+ CustomerOrderFragment.order.get(count).getRemoved()
						+ "'")); // incorrect
				Log.i("sending: ", "name Orders");
				Log.i("sending: ", "value" + "TableId, Title, Price, Modifiables");
				Log.i("sending: ", "data" + "'" + AsyncHelper.getTable() + "',"
						+ "'"
						+ CustomerOrderFragment.order.get(count).getName()
						+ "','"
						+ CustomerOrderFragment.order.get(count).getPrice()
			...
			Map<String, String> params = new HashMap<String, String>();
				...
				RegisterGCM.Post("http://192.184.85.36/csce4444/android/gcookupdate.php", params);
